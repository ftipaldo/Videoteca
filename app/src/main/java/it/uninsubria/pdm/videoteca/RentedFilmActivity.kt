...
class RentedFilmActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        database.child(film_nodeName).get().addOnSuccessListener{
            ...
            binding.btnSpecificFilmCommit.text = "X"

            binding.btnSpecificFilmCommit.setOnClickListener {
                UNrentFilm(film_nodeName, availability)
            }
    ...
    private fun UNrentFilm(film_nodeName:String, availability:String){
        //lock sul db (?)
        //leggi valore availability
        //incrementi
        //scrivi nuovo valore sul db
        //rimuovi child in Users > id > Rentals con l'id del film (yyyy_fffff)
        ...
        database.child(user_nodeName).child("Rentals").child(film_nodeName).removeValue().addOnSuccessListener{
            //Toast.makeText(this, "Film unrented", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener {
            //Toast.makeText(this, "Film NOT unrented", Toast.LENGTH_SHORT).show()
        }
